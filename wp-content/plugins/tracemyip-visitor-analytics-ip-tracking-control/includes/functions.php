...
function tmip_static_urls() {
	...
	define("tmip_wp_learn_knowledge_base", 
			tmip_service_url.'/learn/');
	
	define("tmip_wp_vistr_setup_tutorial", 
			tmip_wp_learn_knowledge_base.'wordpress-visitor-tracking-plugin-1382/');
	...
	*/
	...
	if ($failedNonceCheck<>1) {
		...
		if ($proceedToUpdate and isset($info_update) and 
			...
			} elseif ($postVarVTpos<>$tmip_visit_tracker_position) {
				$tmip_visit_tracker_position=$postVarVTpos;
			}
			if ($allowUpdate) {
			...
			<blockquote>
				...
				<section>
					<ul class="tmip_sett_list_ol">
						<li><span>Full featured individual visitor <a href="'.tmip_home_page_url.'/website-analytics.htm" target="_blank">website statistics</a> IP tracker with individual visitor tagging and activity email alerts</span></li>
			...
			<blockquote>
				...
				<section>
					<ol tmip_sett_list_ul>
						...
						<li><span><b>Select the '.tmip_lang_tracker_icon_ps.'</b> using the drop down menu below and click on the ['.tmip_lang_update_settings.'] button. If you are using Cloudflare with <i>Rocket Loader</i>, login to your Cloudflare dashboard and go to the Speed tab. Scroll down to "Rocket Loader", and toggle the feature off to enable real-time visitor tracking. Here are more details on how to adjust the '.tmip_kb_rock_ldr.' settings.</span></li>
						
						<li><span><b>Verify</b> that a '.tmip_lang_visitr_track_ic.' appears on <b>ALL</b> pages of your WordPress website. If you are a Level 2+ '.tmip_service_Dname.' subscriber, you can enable an "Invisible Tracker" mode by logging in to your'.tmip_service_Dname.' account, clicking on [My Projects] menu and then on the "edit" link for your project.</span></li>

						<li><span><b>Access</b> your '.tmip_service_Dname.' console, login to WordPress and go to [Dashboard] => [TraceMyIP > Reports]</span></li>
					</ol>
					<b>If you need additional help</b> and for a detailed step-by-step tutorial on how to create and install the visitor tracker, please click <a href="'.tmip_wp_vistr_setup_tutorial.'" target="_blank"><b>here</b></a>
				<hr>* <span class="tmip_note_small">If you would like to control visitor access to your pages and/or send one-way messages to specific visitors, first, ensure that the Visitor Tracker code is installed properly and working. Next, go to the Page Tracker menu option under your project, generate a  ['.tmip_lang_page_tr_code.'] and place it into the ['.tmip_lang_page_tr_code.'] input box below.</span>
				</section>
			</blockquote>
	...
	https://www.tracemyip.org/learn/disable-rocket-loader-to-enable-visitor-ip-tracking-936/
...
function tmip_rate_review_section() {
		...
		$output='
			...
			<div class="ratehelp">
				...
				<b>Your kind words will not be forgotten.</b>
				<blockquote>
					As we all realize, WordPress is alive thanks to the support from its users! Yet, the plugins development is what makes WordPress so powerful. As developers, since 2008, we have spent <i>an enormous amount of time</i> to build and maintain '.tmip_service_Nname.' service. And up until recently, we have not asked for reviews. However, it\'s an absolutely vital factor for us in order to maintain the plugin and the service for WordPress.</blockquote>
				<blockquote>
					<b>We support and <u>value</u> our users.</b>
					<i class="fa fa-smile-beam fa-lg" style="color: #02970C; opacity: 0.8;"></i>
				</blockquote>
				If you find '.tmip_service_Nname.' useful, <u>please</u> help and <b>vote 5 stars</b> so we could keep '.tmip_service_Nname.' running. <br><br><b>Thank you</b> <i class="fa fa-'.$thankYouIcon.' fa-lg" style="color: #02970C; opacity: 0.8;"></i>
				<hr>
